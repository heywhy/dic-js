...
var CONTAINER_KEY, NEXT_KEY, generateDICKey, is_function, is_nil, typehintRE;
...
exports.getDICKey = function(callback) {
  return callback[CONTAINER_KEY];
};

exports.generateDICKey = generateDICKey = function() {
  return `${CONTAINER_KEY}-${NEXT_KEY++}`;
};

exports.attachKey = function(factory) {
  var key;
  if (!(key = factory[CONTAINER_KEY])) {
    key = factory[CONTAINER_KEY] = generateDICKey();
  }
  return key;
};
